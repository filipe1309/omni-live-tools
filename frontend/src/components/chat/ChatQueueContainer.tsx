...
export function ChatQueueContainer({ items, title, onRemove, maxHeight = 'calc(100vh - 320px)' }: ChatQueueContainerProps) {
    ...
    <div className="card flex flex-col">
      ...
      >
        {items.length === 0 ? (
          ...
          items.map((item) => (
            <QueueMessage key={item.id} item={item} onRemove={onRemove} />
          ))
        )}
      </div>
    </div>
